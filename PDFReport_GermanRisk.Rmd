...
#get glimpse of german credit data 
'german credit data'
glimpse(german)
```
...
- `r text_spec("Quantitative features(Age,Duration,Amount,InstallmentRatePercentage,etc)", background = "blue", color = "white", bold = T)` .The Data exploration of numerical attributes pointed out the subsequent insights : 
...
```{r ,out.width='.49\\linewidth', fig.align="center", fig.show="hold", message=FALSE, echo=FALSE, fig.cap="\\label{fig:fig2}Credit amount accross credit risk, density and box plots "}
...
- `r text_spec("Qualitative features(CheckingAccountStatus,CreditHistory,Purpose,SavingAccount,etc)", background = "blue", color = "white", bold = T)`.The Data exploration of categorical attributes pointed out the subsequent insights : 
...
Here, in the pdf report we just report figures for the Purpose attribute. See all plots and details in the Rmd report at this [link](https://mono33.github.io/GermanCreditRisk/Report_GermanRisk.html).


```{r ,out.width='.49\\linewidth', fig.align="center", fig.show="hold", message=FALSE, echo=FALSE, fig.cap="\\label{figs:figs3}Credit purpose accross credit worthiness, histogram and balloon plots "}
...
#contingency table of the counts at each combination of factor levels - Purpose attribute
...
- `r text_spec("Multivariate Analysis", background = "blue", color = "white", bold = T)`
...
In the pdf report we just report Multi-panel conditioning for the *Age vs credit Amount for Personal status and sex* . Follow the [rmd report](https://mono33.github.io/GermanCreditRisk/Report_GermanRisk.html) to see all plots and details.


```{r, out.width='70%', echo = FALSE , fig.cap="\\label{fig:fig4}Age vs credit amount for Personal status and sex"}

# bivariate scatterplot of "Age vs credit amount for various purpose"
...
```{r,echo=FALSE,message=FALSE,warning=FALSE,comment=NA,null_prefix=TRUE}
...
However, for the purpose of our study we concluded that it was not useful to normalize all skewed attributes. See all details in the [rmd report](https://mono33.github.io/GermanCreditRisk/Report_GermanRisk.html)
...
```{r, echo=FALSE, message=FALSE,warning=FALSE, null_prefix=TRUE}
...
* `r text_spec("WOE,IV (Information package) :", background = "blue", color = "white", bold = T)`
...
```{r,echo=FALSE,null_prefix=TRUE,comment=NA,message=FALSE,warning=FALSE}
...
* `r text_spec("rfe (recursive feature elimination, caret package):", background = "blue", color = "white", bold = T)`
...
For more details on how we build our recursive feature selection algorithm, follow the rmd report at this [link](https://mono33.github.io/GermanCreditRisk/Report_GermanRisk.html).
...
* `r text_spec("Boruta algorithm:", background = "blue", color = "white", bold = T)`

Boruta algorithm is a wrapper built around the random forest classification algorithm implemented in the R package randomForest (Liaw and Wiener, 2002). It tries to capture all the important, interesting features you might have in your dataset with respect to an outcome variable. To Read all steps of this algorithm follow [Kursa & Rudnicki (2010)](https://www.jstatsoft.org/article/view/v036i11), or rmd report.
...
```{r,echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',out.width='70%',fig.cap="\\label{fig:fig7}Boruta result plot"}
#plot boruta object
...
* A **net value function** $\xi$, which utilizes the unit’s parameters or weights $w$ to summarize input data into a net value, $\upsilon$, as $\upsilon$ = $\xi (x,w)$ . The net value function mimics the behavior of a biological neuron as it aggregates signals from linked neurons into an internal representation. Typically, it takes the form of a *weighted sum*, a *distance*, or a *kernel*.
...
As in Hand & Till (2001) ,it can be show that the AUC is related to the Gini coefficient $({\displaystyle G_{1}})$ by the formula ${\displaystyle G_{1}=2{\mbox{AUC}}-1} G_{1}=2{\mbox{AUC}}-1$ , where: $${\displaystyle G_{1}=1-\sum _{k=1}^{n}(X_{k}-X_{k-1})(Y_{k}+Y_{k-1})} \tag{12}$$
...
```{r,echo=FALSE,message=FALSE,warning=FALSE,comment=NA, null_prefix=TRUE, fig.align='center',out.width= '70%',fig.cap="variable importance- logistic model"}
...
```{r ,echo=FALSE, out.width='.49\\linewidth', fig.align="center",fig.show='hold',fig.cap="ROC curves-Logistic regression models"}
#plot Roc curve
...
```{r ,echo=FALSE,out.width='.49\\linewidth', fig.align="center",fig.show='hold',fig.cap="Precision/recall curves-Logistic regression models"}
...
```{r,echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',fig.cap="rpart plot of decision tree model"}
# Plot the rpart dt_model
prp(dt_model,type=2,extra=1)
```

```{r,echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',fig.cap="fancyrpart plot of decision tree model"}
...
```{r,echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',fig.cap="rpart plot of pruned tree"}
# Plot the rpart pruned.tree
prp(pruned.tree, type = 1, extra = 3, split.font = 2, varlen = -10)
```

```{r,echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',fig.cap="fancyrpart plot of pruned tree"}
#A wrapper for plotting rpart trees using prp
...
```{r, echo=FALSE,message=FALSE,warning=FALSE,comment=NA, null_prefix=TRUE, fig.align='center'}
#Model performance plot
...
```{r ,echo=FALSE, out.width='.49\\linewidth', fig.align="center",fig.show='hold',fig.cap="ROC curves-Decision trees"}
#plot Roc curves
...
```{r ,echo=FALSE,out.width='.49\\linewidth', fig.align="center",fig.show='hold',fig.cap="Precision/recall curves-Decision trees"}
# Plot precision/recall curve
...
```{r, echo=FALSE,message=FALSE,warning=FALSE,comment=NA, null_prefix=TRUE,fig.align='center',out.width='70%',fig.cap="random forest variable importance"}
...
#fit new rf model
set.seed(1)
rf_model.new <- randomForest(Risk ~ Amount+check_acct+Age+Purpose+Duration, data = train, importance=TRUE)
...
```{r ,echo=FALSE, out.width='.49\\linewidth', fig.align="center",fig.show='hold',fig.cap="ROC curves-random forests"}
#plot Roc curves
...
```{r ,echo=FALSE,out.width='.49\\linewidth', fig.align="center",fig.show='hold',fig.cap="Precision/recall curves-random forests"}
# Plot precision/recall curve
...
```{r,echo=FALSE,message=FALSE,warning=FALSE,comment=NA,null_prefix=TRUE, fig.align='center',out.width='70%',fig.cap="important features selected by 10 fold cv"}
...
```{r,echo=FALSE,message=FALSE,warning=FALSE,comment=NA,null_prefix=TRUE, fig.align='center',out.width='70%'}
...
```{r ,echo=FALSE, out.width='.49\\linewidth', fig.align="center",fig.show='hold',fig.cap="ROC curves-SVM"}
#plot Roc curves
...
```{r, echo=FALSE,fig.align='center',out.width= '70%',fig.cap="ROC curves-Neural Networks"}
#plot roc curves
...
```{r, echo=FALSE,fig.align='center', out.width= '70%',fig.cap="Precision/recall curves-Neural Networks"}
# Plot precision/recall curve
...
```{r,echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',out.width='70%',fig.cap="lasso model plot "}
plot(lasso_model)
```
...
```{r, echo=FALSE,fig.align='center',out.width= '70%',fig.cap="ROC curves-Lasso regression"}
#plot roc curves
...
```{r, echo=FALSE,fig.align='center',out.width= '70%',fig.cap="Precision/recall-Lasso regression"}
# Plot precision/recall curve
...
knitr::include_graphics("KSTest.PNG") 
```
...
### Model Performance Comparison - Measures of Accuracy {#mpc}

- `r text_spec("Balanced Accuracy , F1-score, Recall, Precision", background = "blue", color = "white", bold = T)`
...
`cm$byClass[['Balanced Accuracy']]` corresponds to the formula we define first. **( (sensitivity  + specificity) / 2)** . F1-score ,  The **F_meas** function in the caret package computes this summary with beta defaulting to 1 (Irrizary, 2018) .It corresponds to the F1 score formula we defined at the beginning of the paragraph. Also, We can notice that in paragraph  IV.2, formula (8) is the same as formula (11). Then we can get recall metric as sensitivity from the output of the confusion matrix. Instead , for the Precision metric, formula(10) , its value in the confusionMatrix output corresponds to the PPV(positive pred value).Read more [here](https://en.wikipedia.org/wiki/Confusion_matrix)
...
# Colnames 
colnames(model_performance_metric_1) <- c("Models", "Recall", "Precision", "BACC", "F1-score", "Accuracy")

# kable performance table  metrics group1
kable(model_performance_metric_1) %>%
  kable_styling(bootstrap_options = "striped" , full_width = F , position = "center") %>%
  kable_styling(bootstrap_options = "bordered", full_width = F , position ="center") %>%
  column_spec(1,bold = T ,color ="white", background = "blue") %>%
  column_spec(2,bold = T , color ="white", background = "Apricot") %>%
  column_spec(3,bold = T , color ="white", background = "Peach") %>%
  column_spec(4,bold = T ,color ="white", background = "RedOrange" ) %>%
  column_spec(5,bold = T , color ="white", background = "BrickRed") %>%
  column_spec(6,bold =T ,color = "white" , background ="#D7261E") %>%
  footnote(general = "Comparison of Model Performances- all fitted models",
...
- `r text_spec("AUC, KS, GINI", background = "blue", color = "white", bold = T)`
...
kable(model_performance_metric_2) %>%
  kable_styling(bootstrap_options = "striped" , full_width = F , position = "center") %>%
  kable_styling(bootstrap_options = "bordered", full_width = F , position ="center") %>%
  column_spec(1,bold = T ,color ="white", background = "blue" ) %>%
  column_spec(2,bold = T ,color ="white", background = "RedOrange" ) %>%
  column_spec(3,bold = T , color ="white", background = "BrickRed") %>%
  column_spec(4,bold =T ,color = "white" , background ="#D7261E") %>%
  footnote(general = "Comparison of Model Performances- best models",
